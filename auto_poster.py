...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["간병사 자격증 (수강료 전액 지원)", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 국가기술자격 및 평생교육 전문 수험 카운슬러입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "certificate study success",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'certificate study success')
